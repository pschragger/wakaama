...
static size_t prv_encodeUInt(uint64_t data, uint8_t *data_buffer) {
    size_t length = 0;
...
int tlv_parse(const uint8_t *buffer, size_t bufferLen, lwm2m_data_t **dataP) {
    ...
    *dataP = NULL;
    ...
    while (0 != (result = lwm2m_decode_TLV((uint8_t *)buffer + index, bufferLen - index, &type, &id, &dataIndex,
                                           &dataLen))) {
...
static int prv_getLength(int size, lwm2m_data_t *dataP) {
    ...
    for (i = 0; i < size && length != -1; i++) {
        ...
        case LWM2M_TYPE_FLOAT: {
            ...
            if ((cur->value.asFloat < 0.0 - (double)FLT_MAX) || (cur->value.asFloat > (double)FLT_MAX)) {
                data_len = 8;
            } else {
                data_len = 4;
            }
